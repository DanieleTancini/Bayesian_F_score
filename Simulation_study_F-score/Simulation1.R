#In this script we provide the simulation study for synthethic dataset 1
#In the first block is considered the Bayesian logistic model and its frequentist
#In the second block is considered the Bayesian HMM, including the multimodel framework and the model averaging

#Please set at the beginning your WD

#---------------------------
#Bayesian and frequentist logistic model 
#---------------------------
...
for(dim in 31:31){ #set dim in 31:50 if you want to rerun the entire simulation study
  i_new = i_new + 1
  ...
  n = 300 #dimension
  Time = 50 #time
  Alpha_inz = -1 #intercept
  Beta_inz = c(-1,1/2,0) #betas
  set.seed(123)
  P = length(Beta_inz)
  ...
  for(i in 1:n){
    for(t in 1:Time){
      Y[i,t] = rbinom(1, 1, exp(Alpha_inz + X[i,,t]%*%Beta_inz)/(1+exp(Alpha_inz + X[i,,t]%*%Beta_inz)))
    }
  }
  ...
  X_test = X[,,dim]
  Y_test = Y[,dim]
  X = X[,,-c(0:(i_new-1),dim:50)]
  Y = Y[,-c(0:(i_new-1),dim:50)]
  Time = dim(Y)[2]
  ...
  file_ = paste0("Sim_K1",i_new,".Rdata") 
  ...
  f_score_non_bayes[(dim-30)] = (2*table(y_pred_no_bayes,Y_test)[4])/(2*table(y_pred_no_bayes,Y_test)[4] + table(y_pred_no_bayes,Y_test)[3] + table(y_pred_no_bayes,Y_test)[2])
  f_score_bayes[(dim-30)] = (2*table(Y_pred,Y_test)[4])/(2*table(Y_pred,Y_test)[4] + table(Y_pred,Y_test)[3] + table(Y_pred,Y_test)[2])
}
...
for(i_new in 1:1){ #for(i_new in 1:20) if you want to run the entire simulation study
  load(paste0("Sim_K1",i_new,".Rdata"))
...
for(dime in 31:31){ #for(dime in 31:50) for a complete simulation
  i_new = i_new + 1
  ...
  X_test = X[,,dime]
  Y_test = Y[,dime]
  X = X[,,-c(0:(i_new-1),dime:50)]
  Y = Y[,-c(0:(i_new-1),dime:50)]
  Ti = Time = dim(Y)[2]
  ...
  file_ = paste0("Sim_K1_RJMCMC",i_new,".Rdata")
  save.image(file = file_)
  ...
  f_score_bayes_RJMCMC[(dime-30)] = (2*table(Y_pred,Y_test)[4])/(2*table(Y_pred,Y_test)[4] + table(Y_pred,Y_test)[3] + table(Y_pred,Y_test)[2])
...
for(i_new in 1:1){#i_new in 1:20
  load(paste0("Sim_K1_RJMCMC",i_new,".Rdata"))
...
f_score_bayes_RJMCMC_bart_ma = rep(NA,20)

for(i_new in 1:1){ #for(i_new in 1:20) for a complete simulation 
  load(paste0("Sim_K1_RJMCMC",i_new,".Rdata"))
  ...
  Y_test_control = Y_test
  f_score_bayes_RJMCMC_bart_ma[i_new] = (2*table(Y_pred_,Y_test)[4])/(2*table(Y_pred_,Y_test)[4] + table(Y_pred_,Y_test)[3] + table(Y_pred_,Y_test)[2])
...
f_score_bayes_RJMCMC_ma2 = rep(NA,20)
...
for(i_new in 1:1){#for(i_new in 1:20) for a complete simulation
  ...
  f_score_bayes_RJMCMC_ma2[i_new] = (2*table(Y_pred,Y_test)[4])/(2*table(Y_pred,Y_test)[4] + table(Y_pred,Y_test)[3] + table(Y_pred,Y_test)[2])
